...
import { taggedConstraint } from '../../src/inversify';
...
describe('Container', () => {
  ...
  it('Should be able to copy a subset of bindings from a container to another.', () => {
    ...
    expect(container.getAll(weaponServiceIdentifier).length).equal(2);
    expect(container.get(armorServiceIdentifier)).to.be.instanceOf(Helmet);
  });
  ...
  it('Should share singleton instances a binding and its copied binding.', () => {
    ...
    expect(shuriken1.time).to.equal(shuriken2.time);
  });
  ...
  it('Should be able to check is there are tagged bindings available for a given identifier only in current container', () => {
    ...
    expect(
      containerParent.isBoundTagged(Ninja, villageTag, leafVillage),
    ).to.eql(true);
    expect(
  ...
  it('Should bind a service to a container and resolve its dependencies in another container.', () => {
    ...
    expect(service.dependency).to.be.instanceOf(Dependency);
  });
});
